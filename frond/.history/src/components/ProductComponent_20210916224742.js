...
class ProductComponent extends React.Component{
    ...
    componentDidMount(){
        this.getProducts();
        this.state.pageCount = this.state.products? Math.ceil(this.state.products.length)/this.statepageSize: 0;
    ...
    setpaginatedProduct(data,start, end){
        this.state.paginateProduct = _(data).slice(start).take(end).value();
                            ...
                            <ul className="pagination">
                                {
                                    this.state.pages.map((page) => (
                                        <li 
                                            className={
                                                this.state.pages===this.state.currentPage? "page-item active":"page-item"
                                            }
                                        >
                                            <p className="page-link"
                                            onClick={()=>this.pagination(this.state.pages)}
                                            >{page} </p>
                                        </li>
                                    ))
                                }
